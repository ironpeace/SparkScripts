...
case class RunArguments(val hostName: String, 
                       ...
                       val threshold: Float,
                         val stepNum: Int,
                          val output: String,
                         val partNum: Int,
                    val partStrategy: Option[PartitionStrategy],
                        val useBagel: Boolean)
...
object Run extends Logging {
    ...
    def main(arguments: Array[String]) = {
        ...
        val sc = new SparkContext(args.hostName,
                                  "com.dodopipe.PageRank(" + args.inputFile + ")", 
                                  conf)

        if (args.useBagel) {
            BagelPageRank(sc,args.inputFile, args.threshold, args.partNum)
            sys.exit(0)
        }
        val unpartitionedGraph = 
                    GraphLoader.edgeListFile(sc, 
                                             args.inputFile,
                                             minEdgePartitions = args.partNum)
                    .cache()
        ...
        val pr = args.exitCond match {
            ...
            case "withIterationSteps" => 
                GraphxPageRank.withIterationSteps(graph, args.stepNum)
                                            .vertices
                                            .cache()
    ...
    protected def parseArgs(args: Array[String]): RunArguments = { 
        ...
        options.foreach{
            ...
            case ("useBagel", v)     => useBagel = v.toBoolean
